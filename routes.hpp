...
inline void setup_routes()
{
    ...
    Router::add_middleware(
        [](const HttpRequest &request, const std::optional<Json> &, const std::function<HttpResponse()> &next) {
            LOG_DEBUG(std::format("Request: {} {}", method_to_string_view(request.method), request.path));
            auto response = next();
            try {
                std::visit(
                    [&](const auto &body) {
                        if constexpr (std::is_same_v<std::decay_t<decltype(body)>, std::string>) {
                            LOG_DEBUG(std::format("Response: {} ({} bytes)", static_cast<int>(response.status_code),
                                                  body.size()));
                        }
                        else if constexpr (std::is_same_v<std::decay_t<decltype(body)>, HttpStreamData>) {
                            LOG_DEBUG(std::format("Response: {} ({} bytes, streamed)",
                                                  static_cast<int>(response.status_code), body.file_size));
                        }
                        else {
                            LOG_ERROR(std::format("Response: {} (unknown body type)",
                ...
                return HttpResponse(INTERNAL_SERVER_ERROR, Json{{"error", "Internal server error"}}.dump(),
    ...
    Router::add_route(
        GET, "/favicon.ico", [](const HttpRequest &request, const HttpRequestParams &, const std::optional<Json> &) {
            ...
            if (ec) {
                LOG_ERROR(
                    std::format("Failed to get file size for favicon: {} ({})", favicon_path.string(), ec.message()));
                return HttpResponse(INTERNAL_SERVER_ERROR,
                                    prefers_html ? ERROR_500_HTML.data()
                                                 : Json{{"error", "Failed to read favicon"}}.dump(),
                                    content_type);
            }

            auto last_modified = std::filesystem::last_write_time(favicon_path, ec);
            if (ec) {
                LOG_ERROR(std::format("Failed to get last modified time for favicon: {} ({})", favicon_path.string(),
                                      ec.message()));
                return HttpResponse(INTERNAL_SERVER_ERROR,
                                    prefers_html ? ERROR_500_HTML.data()
                                                 : Json{{"error", "Failed to read favicon metadata"}}.dump(),
                                    content_type);
            }

            FileCacheEntry cache_entry;
            // TODO: FIX this static connection_id and request_id
            if (FileCache::get(favicon_path.string(), cache_entry, last_modified, 0, 0)) {
                ...
                if (request.range) {
                    ...
                    if (start >= file_size || start > end || end >= file_size) {
                        LOG_DEBUG(std::format("Invalid range request for favicon: {}-{}, file_size: {}", start, end,
                                              file_size));
                        HttpResponse resp(RANGE_NOT_SATISFIABLE,
                                          prefers_html ? ERROR_416_HTML.data()
                                                       : Json{{"error", "Invalid range"}}.dump(),
                                          content_type);
                    ...
                    response.set_header("Accept-Ranges", "bytes");
                    response.set_header("Last-Modified", format_last_modified(last_modified));
                    LOG_DEBUG(std::format("Serving cached favicon (range): {} (range: {}-{})", favicon_path.string(),
                                          start, end));
                }
                else {
                    LOG_DEBUG(std::format("Serving cached favicon: {} (size: {})", favicon_path.string(),
                                          cache_entry.content.size()));
                }
                return response;
            }
            ...
            file.close();
            // TODO: FIX this static connection_id and request_id
            FileCache::put(favicon_path.string(), content, last_modified, 0, 0);
            ...
            if (request.range) {
                ...
                if (start >= file_size || start > end || end >= file_size) {
                    LOG_DEBUG(
                        std::format("Invalid range request for favicon: {}-{}, file_size: {}", start, end, file_size));
                    HttpResponse resp(RANGE_NOT_SATISFIABLE,
    ...
    Router::add_route(POST, "/echo",
                      [](const HttpRequest &request, const HttpRequestParams &, const std::optional<Json> &) {
                          return make_response(OK, CONTENT_TYPE_PLAIN, request.body);
                      });
    Router::add_route(
        POST, "/json", [](const HttpRequest &request, const HttpRequestParams &, const std::optional<Json> &json_body) {
            if (!json_body) {
                return make_response(BAD_REQUEST, CONTENT_TYPE_JSON,
                                     Json{{"error", "Missing or invalid JSON body"}}.dump());
            }
            try {
                std::string name = json_body->value("name", "Unknown");
                Json response_json = {{"status", "received"}, {"name", name}, {"size", request.body.size()}};
    ...
    Router::add_route(POST, "/form",
                      [](const HttpRequest &request, const HttpRequestParams &, const std::optional<Json> &) {
                          if (request.form_params.empty()) {
                              return make_response(BAD_REQUEST, CONTENT_TYPE_JSON,
                                                   Json{{"error", "No form data or invalid Content-Type"}}.dump());
                          }
                          Json response_body;
    ...
    Router::add_route(GET, "/user/:id",
                      [](const HttpRequest &, const HttpRequestParams &params, const std::optional<Json> &) {
                          Json response_json = {{"id", params.at("id")}, {"message", "User found"}};
                          return make_response(OK, CONTENT_TYPE_JSON, response_json.dump());
                      });
    Router::add_route(
        PUT, "/user/:id",
        [](const HttpRequest &, const HttpRequestParams &params, const std::optional<Json> &json_body) {
            if (!json_body) {
                return make_response(BAD_REQUEST, CONTENT_TYPE_JSON, Json{{"error", "Missing JSON body"}}.dump());
            }
            Json response_json = {{"id", params.at("id")}, {"message", "User updated"}, {"data", *json_body}};
            return make_response(OK, CONTENT_TYPE_JSON, response_json.dump());
        });
    Router::add_route(DELETE, "/user/:id",
                      [](const HttpRequest &, const HttpRequestParams &params, const std::optional<Json> &) {
                          Json response_json = {{"id", params.at("id")}, {"message", "User deleted"}};
                          return make_response(OK, CONTENT_TYPE_JSON, response_json.dump());
                      });
    Router::add_route(
        PATCH, "/user/:id",
        [](const HttpRequest &, const HttpRequestParams &params, const std::optional<Json> &json_body) {
            ...
            Json response_json = {{"id", params.at("id")}, {"message", "User patched"}, {"data", *json_body}};
            return make_response(OK, CONTENT_TYPE_JSON, response_json.dump());
        });
    ...
    [](const WebSocketFrame &frame, ConnectionId conn_id, RequestId req_id) {
        ...
        return frame.payload; // Echo the received payload
    });
}
