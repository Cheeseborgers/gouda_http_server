...
ThreadPool::ThreadPool(const size_t num_threads) : m_stop{false}, m_pending_tasks{0}
{
    for (size_t i = 0; i < num_threads; ++i) {
        m_workers.emplace_back(
            [this, thread_id = i](const std::stop_token &stoken) { worker_loop(stoken, thread_id); });
    }
}
...
void ThreadPool::wait_for_all()
{
    ...
    m_wait_condition.wait(lock, [this] { return m_pending_tasks.load() == 0; });
}
...
void ThreadPool::worker_loop(std::stop_token stoken, const size_t thread_id)
{
    ...
    while (!stoken.stop_requested()) {
...

        {
            ...
            m_condition.wait(lock, stoken,
                             [this, &stoken] { return m_stop || !m_tasks.empty() || stoken.stop_requested(); });
