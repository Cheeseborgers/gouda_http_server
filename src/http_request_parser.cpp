...
std::optional<HttpRequest> HttpRequestParser::parse(std::string_view request, const bool debug, RequestId request_id)
{
     HttpRequest req;
        ...
        if (debug) {
            LOG_DEBUG(std::format("Request[{}]: Parsed first line: {} {} {}",
                                  request_id, method_to_string(req.method), req.path, http_version_to_string(req.version)));
            ...
            if (key_lower == "range") {
                ...
                if (std::regex_match(value_str, range_matches, range_regex)) {
                    try {
                        ...
                        req.range = range;
                        if (debug) {
                            LOG_DEBUG(std::format("Request[{}]: Parsed Range header: bytes={}-{}", request_id, range.start, range.end));
                        }
                    } catch (const std::exception& e) {
                        LOG_ERROR(std::format("Request[{}]: Invalid Range header value: {} ({})", request_id, value, e.what()));
                        return std::nullopt;
                    }
                } else {
